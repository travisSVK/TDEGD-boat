...
public class SeaweedMesh : MonoBehaviour
{
    ...
    private void AddMesh(Vector3 position, float scale)
    {
        for (int i = 0; i < m_SeaweedMeshes[0].triangles.Length; i += 3)
        {
            m_MeshGenerator.AddTriangle(
                position + (m_SeaweedMeshes[0].vertices[m_SeaweedMeshes[0].triangles[i + 0]] * scale),
                position + (m_SeaweedMeshes[0].vertices[m_SeaweedMeshes[0].triangles[i + 1]] * scale),
                position + (m_SeaweedMeshes[0].vertices[m_SeaweedMeshes[0].triangles[i + 2]] * scale));
        }
    }
    ...
    private void AddSeaweedChunk(Vector3 position)
    {
        ...
        int chunkSize = Random.Range(20, 40);
        for (int i = 0; i < chunkSize; ++i)
        {
            ...
            AddMesh(newPostion, Random.Range(0.4f, 1.2f));
        }
    }
    ...
    private void Awake()
    {
        ...
        for (int x = 0; x < 20; ++x)
        {
            ...
            for (int z = 0; z < 10; ++z)
            {
                ...
                if (Mathf.PerlinNoise((x0 / 20.0f) * 10.0f, (z0 / 10.0f) * 10.0f) > 0.76f)
                {
